...
def done_task(request, id, redirect_id):
    ...
    return redirect(completed_tasks, task=redirect_id)

def denied_task(request, id, redirect_id):
    c_task = Completed_task.objects.get(pk=id) # get completed task that should delete
    bot_user = Bot_user.objects.get(user_id=c_task.user_id) # current bot user that created current task
    c_task.delete()
    # send message to user
...
def allow_output(request, pk):
    ...
    bot = telegram.Bot(token=TOKEN)
    try:
        bot.sendMessage(chat_id=user.user_id, text='Ваш баланс пополнен на {}'.format(str(obj.price)))
    except:
        www = 0 # do nothing
    obj.delete()
...
def deny_output(request, pk):
    ...
    bot = telegram.Bot(token=TOKEN)
    try:
        bot.sendMessage(chat_id=user.user_id, text='Ваш запрос на пополнение счета был отменен')
    except:
        www = 0 # do nothing
    obj.delete()
    return redirect(outputs)